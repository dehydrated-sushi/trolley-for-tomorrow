...
### Changed
- Renamed security/jwt.py to security/jwt_handlers.py to avoid shadowing the PyJWT `jwt` package and prevent potential circular import issues
- Updated all imports in app.py and security/tests/test_jwt.py to reference jwt_handlers instead of jwt

### Added
- Security package integrated into team codebase (was previously only in personal branch)
- Copied jwt_handlers.py, rate_limiter.py, and sanitiser.py into New Code backend, replacing empty placeholder files
- Copied full security/tests/ directory (conftest.py, test_jwt.py, test_rate_limiter.py, test_sanitiser.py)
- All 37 security tests passing

### Notes
- The jwt_handlers.py rename is cosmetic; no function signatures or behaviour changed
- generate_access_token() and register_jwt_error_handlers() are the two public exports from jwt_handlers.py
